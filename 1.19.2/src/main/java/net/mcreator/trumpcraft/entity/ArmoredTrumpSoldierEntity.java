...
public class ArmoredTrumpSoldierEntity extends Monster {
	public ArmoredTrumpSoldierEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(TrumpcraftModEntities.ARMORED_TRUMP_SOLDIER.get(), world);
	}

	public ArmoredTrumpSoldierEntity(EntityType<ArmoredTrumpSoldierEntity> type, Level world) {
		super(type, world);
		maxUpStep = 0.6f;
		xpReward = 33;
		setNoAi(false);
	...
	public SoundEvent getDeathSound() {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("trumpcraft:armoreddeath"));
	}
	...
	public void die(DamageSource source) {
		super.die(source);
		DropRocketsProcedure.execute(this);
	}

	public static void init() {
		SpawnPlacements.register(TrumpcraftModEntities.ARMORED_TRUMP_SOLDIER.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(TrumpcraftModEntities.ARMORED_TRUMP_SOLDIER.get(), 180);
	}
