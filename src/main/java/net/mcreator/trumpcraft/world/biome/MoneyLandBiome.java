...
public class MoneyLandBiome extends TrumpcraftModElements.ModElement {
	@ObjectHolder("trumpcraft:moneyland")
	public static final CustomBiome biome = null;
	public MoneyLandBiome(TrumpcraftModElements instance) {
		super(instance, 12);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		BiomeDictionary.addTypes(biome, BiomeDictionary.Type.PLAINS, BiomeDictionary.Type.HILLS, BiomeDictionary.Type.MAGICAL,
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.NONE)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(MoneyBlockBlock.block.getDefaultState(),
							Blocks.EMERALD_BLOCK.getDefaultState(), Blocks.EMERALD_BLOCK.getDefaultState())));
			setRegistryName("moneyland");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.EMERALD_ORE.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.EMERALD_BLOCK.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MoneyBlockBlock.block.getDefaultState().getBlock()
							|| ground == Blocks.EMERALD_BLOCK.getDefaultState().getBlock())
							&& (ground2 == MoneyBlockBlock.block.getDefaultState().getBlock()
									|| ground2 == Blocks.EMERALD_BLOCK.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), Blocks.EMERALD_BLOCK.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == Blocks.EMERALD_BLOCK.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, Blocks.EMERALD_BLOCK.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, Blocks.EMERALD_ORE.getDefaultState(), bbox);
							if (state.getBlock().isAir(state, world, genhPos) || state.getMaterial().blocksMovement() || state.isIn(BlockTags.LEAVES)
									|| state.getBlock() == Blocks.DIAMOND_BLOCK.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.EMERALD_BLOCK.getDefaultState().getBlock()) {
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(-1, genh, 0), Blocks.DIAMOND_BLOCK.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(1, genh, 0), Blocks.DIAMOND_BLOCK.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, -1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, -1), Blocks.DIAMOND_BLOCK.getDefaultState(),
												bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(0, genh, 1)))
										setTreeBlockState(changedBlocks, world, position.add(0, genh, 1), Blocks.DIAMOND_BLOCK.getDefaultState(),
												bbox);
						...
						for (int genh = position.getY() - 3 + height; genh <= position.getY() + height; genh++) {
							...
							for (int genx = position.getX() - k4; genx <= position.getX() + k4; genx++) {
								for (int genz = position.getZ() - k4; genz <= position.getZ() + k4; genz++) {
									...
									if (state.isIn(BlockTags.LEAVES) || state.getBlock() == Blocks.EMERALD_BLOCK.getDefaultState().getBlock()) {
		...
		private void addVines(IWorld world, BlockPos pos, Set<BlockPos> changedBlocks, MutableBoundingBox bbox) {
			setTreeBlockState(changedBlocks, world, pos, Blocks.DIAMOND_BLOCK.getDefaultState(), bbox);
			int i = 5;
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.EMERALD_ORE.getDefaultState().getBlock()
