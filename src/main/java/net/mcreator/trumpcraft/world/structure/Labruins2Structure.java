...
public class Labruins2Structure extends TrumpcraftModElements.ModElement {
	public Labruins2Structure(TrumpcraftModElements instance) {
		super(instance, 68);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 1000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "labruins2"));
						if (template == null)
							return false;
