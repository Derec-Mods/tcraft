...
public class LabRuins3Structure extends TrumpcraftModElements.ModElement {
	public LabRuins3Structure(TrumpcraftModElements instance) {
		super(instance, 69);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 2000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "labruins3"));
						if (template == null)
							return false;
