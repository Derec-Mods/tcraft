...
public class Tree1Structure extends TrumpcraftModElements.ModElement {
	public Tree1Structure(TrumpcraftModElements instance) {
		super(instance, 94);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 500) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "tree1"));
						if (template == null)
							return false;
