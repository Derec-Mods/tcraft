...
public class LabRuins1Structure extends TrumpcraftModElements.ModElement {
	public LabRuins1Structure(TrumpcraftModElements instance) {
		super(instance, 67);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 1000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "labruins1"));
						if (template == null)
							return false;
