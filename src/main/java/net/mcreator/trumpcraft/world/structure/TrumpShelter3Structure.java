...
package net.mcreator.trumpcraft.world.structure;
...
public class TrumpShelter3Structure extends TrumpcraftModElements.ModElement {
	public TrumpShelter3Structure(TrumpcraftModElements instance) {
		super(instance, 21);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 750) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "bunker1"));
						if (template == null)
							return false;
						Rotation rotation = Rotation.values()[random.nextInt(3)];
						Mirror mirror = Mirror.values()[random.nextInt(2)];
						BlockPos spawnTo = new BlockPos(i, j + -9, k);
