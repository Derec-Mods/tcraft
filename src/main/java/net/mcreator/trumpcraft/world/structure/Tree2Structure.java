...
public class Tree2Structure extends TrumpcraftModElements.ModElement {
	public Tree2Structure(TrumpcraftModElements instance) {
		super(instance, 101);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 125) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "tree2"));
						if (template == null)
							return false;
