...
public class Rail1Structure extends TrumpcraftModElements.ModElement {
	public Rail1Structure(TrumpcraftModElements instance) {
		super(instance, 170);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 5000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "rail1"));
						if (template == null)
							return false;
