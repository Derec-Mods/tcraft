...
public class Crater1Structure extends TrumpcraftModElements.ModElement {
	public Crater1Structure(TrumpcraftModElements instance) {
		super(instance, 182);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 7000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "crater1"));
						if (template == null)
							return false;
