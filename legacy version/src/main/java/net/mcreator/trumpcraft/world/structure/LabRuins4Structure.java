...
public class LabRuins4Structure extends TrumpcraftModElements.ModElement {
	public LabRuins4Structure(TrumpcraftModElements instance) {
		super(instance, 70);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 1000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "labruins4"));
						if (template == null)
							return false;
