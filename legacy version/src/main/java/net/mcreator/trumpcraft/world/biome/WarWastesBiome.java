...
import net.mcreator.trumpcraft.entity.DemoUnitEntity;
...
public class WarWastesBiome extends TrumpcraftModElements.ModElement {
	@ObjectHolder("trumpcraft:war_wastes")
	public static final CustomBiome biome = null;
	public WarWastesBiome(TrumpcraftModElements instance) {
		super(instance, 188);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.DESERT).waterColor(4159204).waterFogColor(329011).parent("trumpcraft:wartornplains")
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.COBBLESTONE.getDefaultState()),
									new SimpleBlockStateProvider(Blocks.DEAD_BRAIN_CORAL_BLOCK.getDefaultState()))).baseHeight(6)
			...
			addFeature(GenerationStage.Decoration.UNDERGROUND_ORES,
					Feature.DISK
							.withConfiguration(new SphereReplaceConfig(Blocks.SAND.getDefaultState(), 7, 2,
									Lists.newArrayList(Blocks.PODZOL.getDefaultState(), Blocks.COBBLESTONE.getDefaultState())))
			...
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(TrumpSquad1Entity.entity, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(LivingBombEntity.entity, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(TrumpSquad2Entity.entity, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(TrumpSquad3Entity.entity, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(ArmoredTrumpSoldierEntity.entity, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(DeathSquadGolemEntity.entity, 15, 1, 5));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(DemoUnitEntity.entity, 15, 1, 5));
		}
	}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.PODZOL.getDefaultState().getBlock() || ground == Blocks.COBBLESTONE.getDefaultState().getBlock())
							&& (ground2 == Blocks.PODZOL.getDefaultState().getBlock() || ground2 == Blocks.COBBLESTONE.getDefaultState().getBlock())))
						return false;
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == Blocks.DEAD_BRAIN_CORAL_BLOCK.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, Blocks.DEAD_BRAIN_CORAL_BLOCK.getDefaultState(), bbox);
										}
									}
								}
							}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.COBBLESTONE.getDefaultState().getBlock()
					|| blockType == Blocks.DEAD_BRAIN_CORAL_BLOCK.getDefaultState().getBlock()
					|| blockType == Blocks.PODZOL.getDefaultState().getBlock() || blockType == Blocks.COBBLESTONE.getDefaultState().getBlock();
		}
