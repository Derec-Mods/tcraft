...
public class Crater2Structure extends TrumpcraftModElements.ModElement {
	public Crater2Structure(TrumpcraftModElements instance) {
		super(instance, 183);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 7000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "crater2"));
						if (template == null)
							return false;
