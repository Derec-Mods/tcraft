...
public class LabRuins5Structure extends TrumpcraftModElements.ModElement {
	public LabRuins5Structure(TrumpcraftModElements instance) {
		super(instance, 71);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 5000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "labruins5"));
						if (template == null)
							return false;
