...
public class Trumpshelter2Structure extends TrumpcraftModElements.ModElement {
	public Trumpshelter2Structure(TrumpcraftModElements instance) {
		super(instance, 20);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 750) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "bunker2"));
						if (template == null)
							return false;
