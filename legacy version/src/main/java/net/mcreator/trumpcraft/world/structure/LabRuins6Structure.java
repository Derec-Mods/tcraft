...
public class LabRuins6Structure extends TrumpcraftModElements.ModElement {
	public LabRuins6Structure(TrumpcraftModElements instance) {
		super(instance, 104);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 1000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "labruins6"));
						if (template == null)
							return false;
