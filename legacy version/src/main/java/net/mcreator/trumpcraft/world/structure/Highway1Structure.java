...
public class Highway1Structure extends TrumpcraftModElements.ModElement {
	public Highway1Structure(TrumpcraftModElements instance) {
		super(instance, 22);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 150) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "highway1"));
						if (template == null)
							return false;
