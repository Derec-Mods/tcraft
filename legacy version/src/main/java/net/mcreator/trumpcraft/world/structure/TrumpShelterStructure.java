...
public class TrumpShelterStructure extends TrumpcraftModElements.ModElement {
	public TrumpShelterStructure(TrumpcraftModElements instance) {
		super(instance, 19);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 650) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("trumpcraft", "bunker"));
						if (template == null)
							return false;
